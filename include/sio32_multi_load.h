...
// #define SIO32ML_DI_FUNC_FAST                // SIO Interrupt
// Prohibit Function High Speed Flag (CPU Internal RAM Execution)
#endif
...
extern struct Sio32MultiLoadArea gSio32MultiLoadArea; // 32bit Serial Communication
...
//* To avoid problem with shift clock synchronization set master to
// 256Kbps
//  32 bit serial communication mode first.
//
//* 2Mbps communication with a cable is not guaranteed so do not set
// up.
//* Set already connection already determined with multi-play
// communication
//  to Type.
...
//* After 1 frame wait, set the slave to 256Kbps 32bit serial
// communication
//  mode. Also, until the slave's SIO mode is determined the waits and
//  then communication is started.
//* Slave successively calculates receive data checksum, and checks
// when
//  receive complete.
//* If receive is not completed at time when should be, end receive
// forcibly.
//* After communication ends prohibit communication interrupts, and
// return
