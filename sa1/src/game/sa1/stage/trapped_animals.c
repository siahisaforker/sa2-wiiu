...
#define ANIMAL_TYPE_BOUNCING 1
#define ANIMAL_TYPE_FLYING   2
#elif (GAME == GAME_SA2)
#define ANIMAL_TYPE_STATIC   0
...
#if (GAME == GAME_SA1)
const TrappedAnimalData sAnimsTrappedAnimals[NUM_ANIMAL_ZONES][ANIMAL_VARIANTS_PER_ZONE] = {
    [ZONE_1] = {
        { 192, SA1_ANIM_ANIMAL_SEA_OTTER, 0, ANIMAL_TYPE_STATIC},
        { 196, SA1_ANIM_ANIMAL_KOALA, 0, ANIMAL_TYPE_STATIC},
        { 200, SA1_ANIM_ANIMAL_KANGAROO, 0, ANIMAL_TYPE_BOUNCING},
    },
    [ZONE_2] = {
        { 192, SA1_ANIM_ANIMAL_MOLE, 0, ANIMAL_TYPE_STATIC},
        { 196, SA1_ANIM_ANIMAL_SKUNK, 0, ANIMAL_TYPE_BOUNCING},
        { 200, SA1_ANIM_ANIMAL_ROBIN, 0, ANIMAL_TYPE_FLYING},
    },
    [ZONE_3] = {
        { 192, SA1_ANIM_ANIMAL_LION, 0, ANIMAL_TYPE_STATIC},
        { 196, SA1_ANIM_ANIMAL_PEACOCK, 0, ANIMAL_TYPE_BOUNCING},
        { 200, SA1_ANIM_ANIMAL_PARROT, 0, ANIMAL_TYPE_FLYING},
    }, 
    [ZONE_4] = {
        { 192, SA1_ANIM_ANIMAL_SEA_OTTER, 0, ANIMAL_TYPE_STATIC},
        { 196, SA1_ANIM_ANIMAL_SEAL, 0, ANIMAL_TYPE_STATIC},
        { 200, SA1_ANIM_ANIMAL_PENGUIN, 0, ANIMAL_TYPE_BOUNCING},
    },
    [ZONE_5] = {
        { 192, SA1_ANIM_ANIMAL_ELEPHANT, 0, ANIMAL_TYPE_STATIC},
        { 196, SA1_ANIM_ANIMAL_LION, 0, ANIMAL_TYPE_STATIC},
        { 200, SA1_ANIM_ANIMAL_GORILLA, 0, ANIMAL_TYPE_BOUNCING},
    },
    [ZONE_6] = {
        { 192, SA1_ANIM_ANIMAL_MOLE, 0, ANIMAL_TYPE_STATIC},
        { 196, SA1_ANIM_ANIMAL_DEER, 0, ANIMAL_TYPE_BOUNCING},
        { 200, SA1_ANIM_ANIMAL_RABBIT, 0, ANIMAL_TYPE_BOUNCING},
    }
};
#elif (GAME == GAME_SA2)
static const TrappedAnimalData sAnimsTrappedAnimals[NUM_ANIMAL_ZONES][ANIMAL_VARIANTS_PER_ZONE] = {
    [ZONE_1] = {
...
#if (GAME == GAME_SA1)
    x += Div(COS(animal->unk38 & ONE_CYCLE), 20);
    y += Div(SIN(animal->unk38 & ONE_CYCLE), 20);
#elif (GAME == GAME_SA2)
...
#if ((GAME == GAME_SA2) || defined(BUG_FIX))
    if (zone > (s32)ARRAY_COUNT(sAnimsTrappedAnimals)) {
        zone = ZONE_7;
    }
#endif
...
#if (GAME == GAME_SA1)
    if (type != 0) {
        if (type & ANIMAL_TYPE_BOUNCING) {
            CreateBouncingAnimal(&options);
        } else {
            CreateFlyingAnimal(&options);
        }
    } else {
        CreateStaticAnimal(&options);
    }
#elif (GAME == GAME_SA2)
