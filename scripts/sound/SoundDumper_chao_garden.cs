...
namespace dump_sound
{
    class SoundDumper
    {
        static int MPlayTableAdr = 0x0201A474;
        ...
        static void DumpProgrammableWaveSample(int ofs, int len)
        {
            ...
            sb.Append("\t.align 2\n");
            sb.AppendFormat("\t.global ProgrammableWaveData_{0:X8}\n", ofs);
            sb.AppendFormat("ProgrammableWaveData_{0:X8}:\n", ofs);
        ...
        static string PrintSong(int ofs, int id)
        {
            ...
            if (soundInfo.ContainsKey(ofs))
            {
                ...
                realName = soundInfo[ofs].Substring(pos + 8, 8);
            }
        ...
        static string PrintSongTable(int ofs)
        {
            ...
            for (int i = 0; i < NumSongs; i++)
            {
                ...
                Console.WriteLine(soundInfo[header].Substring(pos + 8, 8));
                sb.AppendFormat("\tsong {0}, {1}, {2}\n", soundInfo[header].Substring(pos + 8, 8), ms, me);
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var adr in soundInfo)
            {
                ...
                if (adr.Value.Contains(".global song"))
                {
                    ...
                    string curr = adr.Value.Substring(pos + 8, 8);
